...
Params::Params(const CommandLine &cl)
{
    ...
    // calculations
    circleSectorOverlapTolerance = static_cast<int>(config.circleSectorOverlapToleranceDegrees
                                                    / 360. * 65536);
    ...
    if (inputFile.is_open())
    {
        ...
        if (content.substr(0, 7) == "VEHICLE")
        {
            ...
            // Create a vector where all information on the Clients can be stored and loop over all
            ...
            while (inputFile >> node)
            {
                ...
                inputFile >> cli[nbClients].coordX >> cli[nbClients].coordY >> cli[nbClients].demand
                    >> cli[nbClients].earliestArrival >> cli[nbClients].latestArrival
                    >> cli[nbClients].serviceDuration;

                // Scale coordinates by factor 10, later the distances will be rounded so we
                // optimize with 1 decimal distances
        ...
        else
        {
            ...
            for (inputFile >> content; content != "EOF"; inputFile >> content)
            {
                ...
                else if (content == "EDGE_WEIGHT_FORMAT")
                {
                    ...
                    {
                        throw std::string(
                            "EDGE_WEIGHT_FORMAT can only be used with EDGE_WEIGHT_TYPE : EXPLICIT");
                    }
                ...
                else if (content == "EDGE_WEIGHT_SECTION")
                {
                    if (!isExplicitDistanceMatrix)
                    {
                        throw std::string("EDGE_WEIGHT_SECTION can only be used with "
                ...
                else if (content == "NODE_COORD_SECTION")
                {
                    ...
                    for (int i = 0; i <= nbClients; i++)
                    {
                        ...
                        {
                            throw std::string(
                                "Clients are not in order in the list of coordinates");
                        }

                        cli[i].custNum--;
                        cli[i].polarAngle = CircleSector::positive_mod(static_cast<int>(
                            32768.
                            * atan2(cli[i].coordY - cli[0].coordY, cli[i].coordX - cli[0].coordX)
                            / PI));
                    }
                }
                ...
                else if (content == "SERVICE_TIME_SECTION")
                {
                    for (int i = 0; i <= nbClients; i++)
                    {
                        ...
                        {
                            throw std::string(
                                "Clients are not in order in the list of service times");
                        }
                    }
                ...
                else if (content == "RELEASE_TIME_SECTION")
                {
                    for (int i = 0; i <= nbClients; i++)
                    {
                        ...
                        {
                            throw std::string(
                                "Clients are not in order in the list of release times");
                        }
                    }
                    // Check if the service duration of the depot is 0
                ...
                else if (content == "TIME_WINDOW_SECTION")
                {
                    ...
                    for (int i = 0; i <= nbClients; i++)
                    {
                        ...
                        {
                            throw std::string(
                                "Clients are not in order in the list of time windows");
                        }
                    }
    ...
    if (config.isDimacsRun)
    {
        ...
        std::cout << "DIMACS RUN for instance name " << instanceName << ", writing solution to "
                  << config.pathSolution << std::endl;
    }
    ...
    {
        // Determine categories of instances based on number of stops/route and whether it has large
        // time windows Calculate an upper bound for the number of stops per route based on capacities
        ...
        // Loop over all clients (excluding the depot) and count the amount of large time windows
        // (greater than 0.7*horizon)
        ...
        std::cout << "----- HasLargeRoutes: " << hasLargeRoutes << ", HasLargeTW: " << hasLargeTW
                  << std::endl;
    ...
    if (!isExplicitDistanceMatrix)
    {
        ...
        for (int i = 0; i <= nbClients; i++)
        {
            ...
            for (int j = i + 1; j <= nbClients; j++)
            {
                ...
                double d = std::sqrt(
                    (cli[i].coordX - cli[j].coordX) * (cli[i].coordX - cli[j].coordX)
        ...
        for (int j = 1; j <= nbClients; j++)
        {
            if (i != j)
            {
                ...
                orderProximity.emplace_back(
                    timeIJ
                        + std::min(proximityWeightWaitTime
    ...
    {
        throw std::string("The distances are of very small or large scale. This could impact "
                          "numerical stability. Please rescale the dataset and run again.");
    }
    if (maxDemand < 0.1 || maxDemand > 100000)
    {
        throw std::string("The demand quantities are of very small or large scale. This could "
                          "impact numerical stability. Please rescale the dataset and run again.");
    }
...
double Params::getTimeElapsedSeconds()
{
    ...
    {
        std::chrono::duration<double> wctduration = (std::chrono::system_clock::now()
                                                     - startWallClockTime);
...
void Params::SetCorrelatedVertices()
{
    ...
    for (int i = 1; i <= nbClients; i++)
    {
        ...
        {
            // If i is correlated with j, then j should be correlated with i (unless we have asymmetric
            // problem with time windows) Insert vertices in setCorrelatedVertices, in the order of
            // orderProximity, where .second is used since the first index correponds to the depot
