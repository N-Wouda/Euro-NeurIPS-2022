...
bool find_cycle(AB_graph &ab_graph, Route &cycle, std::vector<Clients> &visited, bool parent, Client current){
    Clients next = {-1};
    if(current == 0 && parent){
    ...
    for(auto &next_client : next){
        ...
        bool cycleFound = find_cycle(ab_graph, cycle, visited, !parent, next_client);
        if(cycleFound){
            cycle.push_back(current);
            return 1;
        }
    ...
    XorShift128 &rng)
{
    ...
    for(Route route : parents.second->getRoutes()){
        if(!route.empty()){
            ...
            bool found = 0;
            for(auto it = ab_graph.A_from_depot.begin(); it != ab_graph.A_from_depot.end(); it++){
                if(*it == route.front()){
    ...
    for(size_t idx = 0; idx < AB_cycle.size() - 1; idx++){
        ...
        if(current == 0){
            for(auto it = child_from_depot.end(); it != child_from_depot.begin(); ){
                it--;
                if(*it == next){
