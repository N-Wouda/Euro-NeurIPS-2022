...
void Population::doLocalSearchAndAddIndividual(Individual *indiv)
{
    ...
    // With a certain probability, repair half of the solutions by increasing the penalties for
    // infeasibilities (w.r.t. capacities and time warps) in a new Local Search in case of infeasibility
...
void Population::generatePopulation()
{
    ...
    // Note we actually set initial penalty in Params.cpp but by setting it here we also reset it
    // when resetting the population (probably not ideal but test before changing)
    ...
    for (int i = 0; i < nofNearestIndividualsToGenerate; i++)
    {
        ...
        int toleratedCapacityViolation = i == 0
                                             ? 0
                                             : params->rng() % (maxToleratedCapacityViolation + 1);
    ...
    std::cout << "Generated " << nofNearestIndividualsToGenerate << " individuals using Nearest"
              << std::endl;
    printState(-1, -1);
    ...
    // Output that some individuals have been created
    std::cout << "Generated " << nofFurthestIndividualsToGenerate << " individuals using Furthest"
              << std::endl;
    printState(-1, -1);
    ...
    // Output that some individuals have been created
    std::cout << "Generated " << nofSweepIndividualsToGenerate << " individuals using Sweep"
              << std::endl;
    printState(-1, -1);
    ...
    // Output that some individuals have been created
    std::cout << "Generated " << nofRandomIndividualsToGenerate << " individuals Randomly"
              << std::endl;
    printState(-1, -1);
}
...
bool Population::addIndividual(const Individual *indiv, bool updateFeasible)
{
    ...
    // Create a copy of the individual and update the proximity structures calculating
        ...
        && indiv->myCostSol.penalizedCost < bestSolutionRestart.myCostSol.penalizedCost - MY_EPSILON)
    {
            ...
            < bestSolutionOverall.myCostSol.penalizedCost - MY_EPSILON)
        {
            ...
            searchProgress.push_back(
                {params->getTimeElapsedSeconds(), bestSolutionOverall.myCostSol.penalizedCost});
            if (params->config.isDimacsRun)
            {
                ...
                // Print solution for processing by the controller (after output is written since
                // controller may terminate program!) Note: delay for writing is negligible
...
void Population::updateBiasedFitnesses(SubPopulation &pop)
{
    ...
    else
    {
        ...
        for (int i = 0; i < static_cast<int>(pop.size()); i++)
        {
            ...
            {
                pop[ranking[i].second]->biasedFitness = fitRank
                                                        + params->config.diversityWeight * divRank;
            }
            else
            {
                pop[ranking[i].second]->biasedFitness = fitRank
...
void Population::managePenalties()
{
    ...
    double fractionFeasibleLoad = static_cast<double>(std::count(listFeasibilityLoad.begin(),
    ...
    {
        params->penaltyCapacity = std::min(params->penaltyCapacity * params->config.penaltyBooster,
                                           100000.);
    }
    ...
    // Setting some bounds [0.1,100000] to the penalty values for safety
    double fractionFeasibleTimeWarp = static_cast<double>(std::count(listFeasibilityTimeWarp.begin(),
    ...
    {
        params->penaltyTimeWarp = std::min(params->penaltyTimeWarp * params->config.penaltyBooster,
                                           100000.);
    }
    ...
    // If needed, reorder the individuals in the infeasible subpopulation since the penalty values
    // have changed (simple bubble sort for the sake of simplicity)
    for (int i = 0; i < static_cast<int>(infeasibleSubpopulation.size()); i++)
    {
...
Individual *Population::getBinaryTournament()
{
    ...
    // Pick a first random number individual from the total population (of both feasible and
    ...
    // Pick a second random number individual from the total population (of both feasible and
    // infeasible individuals)
...
{
    // Print the number of iterations, the number of iterations since the last improvement, and the
    // running time
    ...
    // If there is at least one feasible solution, print the number of feasible solutions, the cost
    // of the best feasible solution, and the average cost of the feasible solutions
    ...
    // If there is at least one infeasible solution, print the number of infeasible solutions, the
    // cost of the best infeasible solution, and the average cost of the infeasible solutions
    ...
    // Print the diversity of both pools of solutions, the average load- and time warp feasibilities
    // of the last 100 solutions generated by LS, and the penalties for the capacit and the time warp
    std::printf(" | Div %.2f %.2f",
                getDiversity(feasibleSubpopulation),
                getDiversity(infeasibleSubpopulation));
    std::printf(" | Feas %.2f %.2f",
                static_cast<double>(
                    std::count(listFeasibilityLoad.begin(), listFeasibilityLoad.end(), true))
                    / static_cast<double>(listFeasibilityLoad.size()),
                static_cast<double>(
                    std::count(listFeasibilityTimeWarp.begin(), listFeasibilityTimeWarp.end(), true))
                    / static_cast<double>(listFeasibilityTimeWarp.size()));
...
{
    // The diversity of the population: The average of the averageBrokenPairsDistanceClosest over
...
{
    // The average cost of the population: The average of the penalizedCost over the best "mu"
...
void Population::exportBKS(std::string fileName)
{
        ...
        && (!readOK || bestSolutionOverall.myCostSol.penalizedCost < readCost - MY_EPSILON))
    {
        ...
        std::cout << "----- NEW BKS: " << bestSolutionOverall.myCostSol.penalizedCost << " !!!"
                  << std::endl;
...
void Population::logSolution(int nbIter, std::ofstream &myfile, Individual *indiv)
{
    ...
    myfile << indiv->myCostSol.distance << ";" << indiv->myCostSol.capacityExcess << ";"
           << indiv->myCostSol.timeWarp << ";";
...
{
    // Create lists for the load feasibility of the last 100 individuals generated by LS, where all
    // feasibilities are set to true
