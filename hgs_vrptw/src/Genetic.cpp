...
void Genetic::run(int maxIterNonProd, int timeLimit)
{
    ...
    for (int nbIter = 0; nbIterNonProd <= maxIterNonProd && !params->isTimeLimitExceeded(); nbIter++)
    {
        ...
        // Log the current population to a .csv file every logPoolInterval iterations (if
        // logPoolInterval is not 0)
...
Individual *Genetic::crossoverSREX(std::pair<const Individual *, const Individual *> parents)
{
    ...
    while (improved)
    {
        ...
        const int differenceALeft
            = static_cast<int>(
                  std::count_if(parents.first->chromR[(startA - 1 + nOfRoutesA) % nOfRoutesA].begin(),
        ...
        const int differenceARight
            = static_cast<int>(
                  std::count_if(parents.first->chromR[(startA + nOfMovedRoutes) % nOfRoutesA].begin(),
        ...
        const int bestDifference = std::min(
            {differenceALeft, differenceARight, differenceBLeft, differenceBRight});
    ...
    for (int c : unplannedTasks)
    {
        ...
        for (int r = 0; r < params->nbVehicles; r++)
        {
            ...
            for (int i = 1; i < static_cast<int>(offspring->chromR[r].size()); i++)
            {
                ...
                if (params->cli[offspring->chromR[r][i - 1]].earliestArrival + newDistanceToInsert
                        < latestArrival
                           ...
                           < params->cli[offspring->chromR[r][i]].latestArrival)
                {
                    distanceDelta = newDistanceToInsert + newDistanceFromInsert
                                    - params->timeCost.get(offspring->chromR[r][i - 1],
            ...
            if (params->cli[offspring->chromR[r].back()].earliestArrival + newDistanceToInsert
                < latestArrival)
            {
        ...
        offspring->chromR[bestLocation.first].insert(offspring->chromR[bestLocation.first].begin()
                                                         + bestLocation.second,
                                                     c);
    }
}
