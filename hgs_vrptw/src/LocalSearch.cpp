...
bool operator==(const TimeWindowData &twData1, const TimeWindowData &twData2)
{
    return twData1.firstNodeIndex == twData2.firstNodeIndex
           && twData1.lastNodeIndex == twData2.lastNodeIndex && twData1.duration == twData2.duration
...
void LocalSearch::constructIndividualBySweep(int fillPercentage, Individual *indiv)
{
    ...
    for (int r = 0; r < static_cast<int>(nodeIndicesPerRoute.size()); r++)
    {
        ...
        for (int idx : nodeIndicesPerRoute[r])
        {
            ...
            if ((nodesToInsert[idx].twData.latestArrival - nodesToInsert[idx].twData.earliestArrival)
                    * 2
            ...
            for (int j = 0; j <= routes[r].nbCustomers; j++)
            {
                ...
                double insertionCost
                    = params->timeCost.get(prev->cour,
                      ...
                      + params->timeCost.get(nodesToInsert[nodeIndicesToInsertLongTw[i]].clientIdx,
                                             prev->next->cour)
                                                   ...
                                                   Individual *indiv)
{
    ...
    for (int r = 0; r < static_cast<int>(routes.size()) && unassignedNodeIndices.size() > 0; r++)
    {
        ...
        if (useSeedClientFurthestFromDepot)
        {
            ...
            {
                double insertionCost = params->timeCost.get(routes[r].depot->cour,
        ...
        while (insertedNode)
        {
            ...
            for (int idx : unassignedNodeIndices)
            {
                ...
                for (int j = 0; j <= routes[r].nbCustomers; j++)
                {
                    ...
                    TimeWindowData routeTwData = MergeTWDataRecursive(prev->prefixTwData,
                    ...
                    // Compute insertion cost
                    double insertionCost = params->timeCost.get(prev->cour,
                                                                nodesToInsert[idx].clientIdx)
                                           + params->timeCost.get(nodesToInsert[idx].clientIdx,
                                                                  prev->next->cour)
                                           - params->timeCost.get(prev->cour, prev->next->cour);
    ...
    if (unassignedNodeIndices.size() > 0)
    {
        ...
        Node *prevNode = depotsEnd[lastRouteIdx]
        ...
        for (int posU = 0; posU < params->nbClients; posU++)
        {
            ...
            for (const auto &v : correlated)
            {
                                               ...
                                               // modified since last move evaluations for nodeU
                {
                    ...
                    // very common to find improving moves of different types for the same node pair)
        ...
        if (!neverIntensify && searchCompleted && (alwaysIntensify || runLS_INT))
        {
            for (int rU = 0; rU < params->nbVehicles; rU++)
            {
                ...
                for (int rV = 0; rV < params->nbVehicles; rV++)
                {
                    ...
                    if (!CircleSector::overlap(routeU->sector,
                                               routeV->sector,
...
bool LocalSearch::MoveSingleClient()
{
    ...
    else
    {
        ...
        else
        {
            ...
            routeUTwData = MergeTWDataRecursive(nodeV->prefixTwData,
                                                nodeU->twData,
        ...
        costSuppU += penaltyExcessLoad(routeU->load) + penaltyTimeWindows(routeUTwData)
                     - routeU->penalty;
    }
...
bool LocalSearch::SwapTwoSingleClients()
{
    ...
    else
    {
        ...
        else
        {
            ...
            routeUTwData = MergeTWDataRecursive(nodeV->prev->prefixTwData,
                                                nodeU->twData,
                                                getRouteSegmentTwData(nodeY, nodeU->prev),
                                                nodeV->twData,
                                                nodeX->postfixTwData);
...
bool LocalSearch::TwoOptWithinTrip()
{
    ...
    double cost = params->timeCost.get(nodeUIndex, nodeVIndex)
                  + params->timeCost.get(nodeXIndex, nodeYIndex)
                  - params->timeCost.get(nodeUIndex, nodeXIndex)
                  - params->timeCost.get(nodeVIndex, nodeYIndex) + nodeV->cumulatedReversalDistance
...
bool LocalSearch::RelocateStar()
{
    ...
    for (nodeU = routeU->depot->next; !nodeU->isDepot; nodeU = nodeU->next)
    {
        ...
        const TimeWindowData routeUTwData = MergeTWDataRecursive(nodeU->prev->prefixTwData,
                                                                 nodeX->postfixTwData);
        ...
        {
            const TimeWindowData routeVTwData = MergeTWDataRecursive(V->prefixTwData,
                                                                     nodeU->twData,
...
void LocalSearch::preprocessInsertionsWithTW(Route *R1, Route *R2)
{
    ...
    for (Node *U = R1->depot->next; !U->isDepot; U = U->next)
    {
        ...
        if (R1->isDeltaRemovalTWOutdated)
        {
            ...
            U->deltaRemovalTW = params->timeCost.get(U->prev->cour, U->next->cour)
                                - params->timeCost.get(U->prev->cour, U->cour)
                                - params->timeCost.get(U->cour, U->next->cour)
                                                 ...
                                                 const TimeWindowData &twData2)
{
    ...
    mergedTwData.earliestArrival = std::max(twData2.earliestArrival - delta, twData1.earliestArrival)
                                   - deltaWaitTime;
    mergedTwData.latestArrival = std::min(twData2.latestArrival - delta, twData1.latestArrival)
                                 + deltaTimeWarp;
...
void LocalSearch::updateRouteData(Route *myRoute)
{
    ...
    {
        myRoute->polarAngleBarycenter = atan2(cumulatedY / static_cast<double>(myRoute->nbCustomers)
                                                  - params->cli[0].coordY,
