...
void Split::generalSplit(Individual *indiv, int nbMaxVehicles)
{
    ...
    maxVehicles = std::max(nbMaxVehicles,
                           static_cast<int>(
                               std::ceil(params->totalDemand / params->vehicleCapacity)));
...
int Split::splitSimple(Individual *indiv)
{
    ...
    // This code has been maintained as it is very simple and can be easily adapted to a variety of
    // constraints, whereas the O(n) Split has a more restricted application scope
    if (params->isDurationConstraint)
    {
        ...
        for (int i = 0; i < params->nbClients; i++)
        {
            ...
            for (int j = i + 1; j <= params->nbClients && load <= 1.5 * params->vehicleCapacity; j++)
            {
                ...
                // Calculate the cost when this client returns to the depot, including a penalty for
                // possible capacity violations
                double cost = distance + cliSplit[j].dx_0
                              + params->penaltyCapacity
                                    * std::max(load - params->vehicleCapacity, 0);

                // If this leads to lower potential, update to this lower potential, and set the
                // predecessor of j to be i
    ...
    // Check if the cost of the last client is still very large. In that case, the Split algorithm
    // did not reach the last client
    ...
    // First clear some chromR vectors. In practice, maxVehicles equals nbVehicles. Then, this loop
    // is not needed and the next loop starts at the last index of chromR
...
int Split::splitLF(Individual *indiv)
{
    ...
    if (params->isDurationConstraint)
    {
        for (int k = 0; k < maxVehicles; k++)
        {
            for (int i = k; i < params->nbClients && potential[k][i] < 1.e29; i++)
            {
                ...
                int serviceDuration = 0;
                int distance = 0;
                for (int j = i + 1; j <= params->nbClients && load <= 1.5 * params->vehicleCapacity;
    ...
    else  // MAIN ALGORITHM -- Without duration constraints in O(n), from "Vidal, T. (2016). Split
          // algorithm in O(n) for the capacitated vehicle routing problem. C&OR"
    {
        Trivial_Deque queue = Trivial_Deque(params->nbClients + 1, 0);
        for (int k = 0; k < maxVehicles; k++)
        {
            // in the Split problem there is always one feasible solution with k routes that reaches
            // the index k in the tour.
            queue.reset(k);
