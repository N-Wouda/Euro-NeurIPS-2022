...
class ControllerEnvironment(Environment):
    ...
    def reset(
        ...
        assert (
            seed is None
        ), "Argument seed must be None for controller environment as it is decided by controller"
        assert (
            instance is None
        ), "Argument instance must be None for controller environment as it is decided by controller"
        assert (
            epoch_tlim is None
        ), "Argument epoch_tlim must be None for controller environment as it is decided by controller"
        assert (
            is_static is None
        ), "Argument is_static must be None for controller environment as it is decided by controller"
    ...
    def _request(self, action: str, data=None):
        self.write_pipe.write(
            tools.json_dumps_np({"action": action, "data": data})
        )
        self.write_pipe.write("\n")
